...
func TestDecisionEngineMVP(t *testing.T) {
	cases := []struct {
		...
		{
			name:       "git push dispara snapshot (PUBLISH)",
			cmd:        "git",
			args:       []string{"push", "origin", "main"},
			exitCode:   0,
			wantEvent:  EventPublish,
			wantAction: ActionTriggerSnapshot,
		},
		{
			name:       "git push falhando ainda dispara snapshot",
			cmd:        "git",
			args:       []string{"push"},
			exitCode:   1,
			wantEvent:  EventPublish,
		...
		{
			name:       "go build passando nao dispara acao",
			cmd:        "go",
			args:       []string{"build", "./..."},
			exitCode:   0,
			wantEvent:  EventBuild,
			wantAction: ActionNone,
		},
		{
			name:       "go build falhando dispara analyze",
			cmd:        "go",
			args:       []string{"build", "./..."},
			exitCode:   1,
			wantEvent:  EventBuild,
		...
		{
			name:       "docker build falhando dispara analyze",
			cmd:        "docker",
			args:       []string{"build", "-t", "x", "."},
			exitCode:   1,
			wantEvent:  EventBuild,
			wantAction: ActionTriggerAnalyze,
		},
		{
			name:       "npm run build passando nao dispara acao",
			cmd:        "npm",
		...
		{
			name:       "cargo build falhando dispara analyze",
			cmd:        "cargo",
			args:       []string{"build"},
			exitCode:   2,
			wantEvent:  EventBuild,
